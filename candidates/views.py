...
# @api_view(['POST'])
# def apply_for_job(request, candidate_id, post_id):
...
#         application = Application.objects.create(post=post, candidate=candidate, status=application_status, additional_documents=additional_documents) 
#         application.save() 
...
def apply_for_job(request):
    ...
    if serializer.is_valid():
        candidate_id = serializer.validated_data.get('candidate')
        post_id = serializer.validated_data.get('post')  # Get the ID of the post
        
        # Debugging: Print the extracted IDs
        ...
        try:
            ...
            # Debugging: Print candidate and post objects
